...
public final class Launcher extends Application {
                ...
                if (NativeUtils.USE_JNA && Gdi32.INSTANCE != null && User32.INSTANCE != null) {
                    ...
                    if (pointer != null) {
                        try {
                            ...
                            int selectedFrameRate = selectDetectedAnimationFrameRate(refreshRate);
                            LOG.info("Detected refresh rate: " + refreshRate + "Hz; JavaFX pulse: "
                                    + selectedFrameRate + "Hz");
                            System.getProperties().putIfAbsent(
                                    "javafx.animation.pulse", String.valueOf(selectedFrameRate));
                        } finally {
    ...
    static int selectDetectedAnimationFrameRate(int refreshRate) {
        return Math.max(60, refreshRate);
    }
