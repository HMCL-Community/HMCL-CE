...
public final class HMCLGameRepository extends DefaultGameRepository {
    ...
    private InstanceGameSettingsLoadResult loadGameSettingsFile(Path file) {
        ...
        try {
            ...
            switch (schemaResult.status()) {
                ...
                case UNEXPECTED_ID -> LOG.warning("Ucepected instance game settings schema. Expected: "
    ...
    /// Queues instance-specific game settings for persistence.
    ///
    /// Settings for an instance whose manifest has not yet been written are persisted
    /// synchronously, so deleting a cancelled installation cannot race a deferred save
    /// that recreates its version directory.
    ///
    /// @param instanceId instance whose settings should be saved
    public void saveGameSettings(GameInstanceID instanceId) {
        ...
        if (!hasInstance(instanceId)) {
            try {
                ...
                LOG.warning("Failed to save initial instance game settings for " + instanceId, e);
            }
            return;
        }
