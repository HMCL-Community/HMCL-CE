...
/// Serializes enum constants as lower-case stable identifiers and reads them case-insensitively.
///
...
/// Maps lower-case stable identifiers to their corresponding constants for lookup during deserialization.
    ...
    private LowerCaseEnumTypeAdapter(Class<E> enumClass) {
        ...
        for (E constant : enumClass.getEnumConstants()) {
            lowercaseToConstant.put(constant.toString().toLowerCase(Locale.ROOT), constant);
        }
    ...
    public void write(JsonWriter out, @Nullable E value) throws IOException {
        out.value(value != null ? value.toString().toLowerCase(Locale.ROOT) : null);
    }

    /// Converts a JSON primitive into an enum constant by matching stable identifiers case-insensitively.
    ///
    ...
    /// Converts an enum constant into its lower-case stable identifier representation.
    ///
    ...
    public static <E extends Enum<E>> JsonElement toJsonTree(@Nullable E value) {
        return value != null ? new JsonPrimitive(value.toString().toLowerCase(Locale.ROOT)) : JsonNull.INSTANCE;
    }
}
